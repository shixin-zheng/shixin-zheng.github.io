...
A blank topic cell for a numbered lecture is filled from the typed lecture notes, so the
CSV only has to carry the calendar. The CSV's `reading` column is read but not published:
the site does not print Devore section numbers, which went stale against the notes.

A course whose meta carries `topics_as_taught` publishes a lecture's topic only once that
...
def typed_lectures() -> dict[int, str]:
    """lecture number -> title, read from the typed notes. The third argument of
    \\lecture is the Devore sections; the website does not publish those."""
    found = {}
...
def build(key: str, spec: dict, lectures: dict[int, tuple[str, str]]) -> pathlib.Path:
    ...
    out = [
        ...
        "# Edit the CSV, not this file. A title left blank in the CSV is filled from the",
        "# typed lecture notes; Devore section numbers are not published at all.",
    ...
    for week, group in weeks:
        ...
        for r in group:
            ...
            if r["lecture"].isdigit():
                ...
                # borrow their titles; other terms are numbered differently.
                if spec.get("notes_numbering"):
                    title = title or lectures.get(n, "")
